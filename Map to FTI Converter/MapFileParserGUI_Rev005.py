...
class MapFileCSVParser(Frame):
    ...
    def Process(self):
        ...
        if infile == "":
            ...
            if self.symbolsloaded:
                num_sym = outputCSV(outfile,Process(infile,self.symbols.getactivelist()),self.menu.SplitLarge.get())
            else:
                num_sym = outputCSV(outfile,Process(infile,loadSymbols(symfile)),self.menu.SplitLarge.get())
            tkMessageBox.showinfo("Task Complete","Output File Generated\n%s Symbols Generated" %(str(num_sym)))
    ...
    def save_settings(self):
        ...
        if self.menu.SplitLarge.get():
            file.write("SplitLarge,True\n")
        else:
            file.write("SplitLarge,False\n")
        file.close()
    def open_settings(self):
        ...
        for line in projectfile:
            ...
            if data[0] == "SplitLarge":
                if data[1] == "True":
                    self.menu.SplitLarge.set(True)
                elif data[1] == "False":
                    self.menu.SplitLarge.set(False)

class AppMenu(Menu):
    
    def __init__(self,parent,app):
        self.SplitLarge = BooleanVar()
        ...
        self.filemenu.add_checkbutton(label="Split Large Symbols",onvalue=True, offvalue=False, variable=self.SplitLarge)
        self.filemenu.add_separator()
    ...
    def help(self):
        import webbrowser
        help_path = os.path.join(os.getcwd(),"Help","Help.htm")
